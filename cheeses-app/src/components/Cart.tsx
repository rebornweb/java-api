      ...
      {cartItems.length === 0 ? (
        ...
        cartItems.map(item => (
          <Box key={item.cheese.id} borderBottom="1px" borderColor="gray.200" py={2}>
            <Text>{item.cheese.content}: ${item.cheese.pricePerKilo}/kg</Text>
            <Text>Total weight - {item.quantity} kgs </Text>
            <Text>Price: ${(item.cheese.pricePerKilo * item.quantity).toFixed(2)}</Text>
          </Box>
        ))
      )}
      <Text fontWeight="bold" mt={4}>Total Price: ${getTotal().toFixed(2)}</Text>
      <Button mt={4} ml={2} colorScheme="red" onClick={resetCart}>Reset Cart</Button> 
    </Box>
  );
};
